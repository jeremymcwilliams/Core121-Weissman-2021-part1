...
Now it's your turn. Can you find the mean, median, and standard deviation of the "orb width(cm)" column?

```{r}

# Create a variable that targets the swimming column
...
Lastly, it's important to know how to load a data set into RStudio Server, so you can then use R to run statistics (and later create data visualizations) on that data. 

First, download the data for Oregon from the Covid Tracking Project byt putting the following URL in a browser address bar:

https://covidtracking.com/data/download/oregon-history.csv

Once that has downloaded to your computer, upload it to RStudio Server by doing the following:
...
orCovid<-read_csv("oregon-history.csv")
...
Jeremy is also hosting R office hours from 3-4pm on Tuesdays and Thursdays, or by appointment: https://zoom.us/j/6475828456
